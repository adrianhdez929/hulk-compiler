...
private:
    static void parseProduction(const std::string& line, Grammar& g, std::unordered_map<std::string, std::shared_ptr<Symbol>>& symbols) {
        ...
        //             if (std::holds_alternative<std::shared_ptr<SymbolNode>>(arg)) {
        //                 anyArgs.push_back(std::get<std::shared_ptr<SymbolNode>>(arg));
        //             } else {
        //                 // Default case for Node
        ...
        AttrProd prod = AttrProd(left, Sentence(rightSymbols), semanticAction);//wrappedAction);
